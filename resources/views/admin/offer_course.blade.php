  ...
  <div class="flex-1 flex flex-col overflow-hidden">
    ...
    <main class="p-8 overflow-x-auto">
      ...
      <div class="mb-10 flex justify-between">
        <h2 class="text-3xl font-bold text-gray-800 underline decoration-blue-400">Offer Course</h2>
        <a href="{{ route('add.offer.course') }}" class="bg-green-500 px-5 py-2 text-white rounded hover:bg-green-600">
          <i class="fa-solid fa-plus"></i> Add Course
        </a>
      </div>
      <!-- Courses Table -->
      <div class="bg-white shadow m-6 rounded-lg overflow-hidden font-sans">
          ...
          <tbody class="divide-y divide-gray-200 text-gray-800">
            @foreach ($courses as $course)
              <tr class="hover:bg-gray-50 align-top">
                ...
                <td class="px-4 py-3 font-medium max-w-[150px] truncate">{{ $course->course_name }}</td>
                ...
                <td class="px-4 py-3">₹{{ $course->course_price }}</td>
...
<script>
  function openConfirmModal(courseId) {
      ...
      form.action = `/offerd/course/${courseId}`; // Adjust the route if needed
