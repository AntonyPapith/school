...
<div class="flex-1 flex flex-col overflow-hidden">
  ...
  <main class="p-8 w-full">
    ...
    <div class="mb-10 flex justify-between items-center flex-wrap gap-4">
      <h2 class="text-3xl font-bold text-gray-800 underline decoration-blue-400">Student Details</h2>
      <a href="{{ route('student.details.add') }}" 
         class="bg-green-500 px-5 py-2 text-white rounded hover:bg-green-600 whitespace-nowrap">
        <i class="fa-solid fa-plus"></i> Add Student
      </a>        
    </div>

    <!-- Students Table -->
    <div class="bg-white shadow m-6 rounded-lg overflow-auto font-sans w-full">
      <table class="min-w-full text-base border-collapse">
        <thead class="bg-gray-100">
          <tr>
            ...
            <th class="px-4 py-3 text-left font-bold text-blue-900">Course</th>
            <th class="px-4 py-3 text-left font-bold text-blue-900">DOB</th>
            <th class="px-4 py-3 text-left font-bold text-blue-900">Gender</th>
            ...
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-4 py-2 whitespace-nowrap">
                  <div class="flex gap-2">
                      ...
                      <form action="{{ route('student.reject', $student->id) }}" method="POST">
                          @csrf
              ...
              <td class="px-4 py-2 whitespace-nowrap">
                  <div class="flex gap-2">
                      <a href="{{ route('student.details.edit', $student->id) }}" class="bg-yellow-400 hover:bg-yellow-500 text-white px-3 py-1 rounded text-xs font-medium">✏️ Edit</a>
                      <form action="{{ route('student.details.destroy', $student->id) }}" method="POST">
                          @csrf
                          @method('DELETE')
                          <button type="button" onclick="openConfirmModal('{{ $student->id }}')" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs font-medium">🗑️ Delete</button>
                      </form>
                  </div>
              </td>
            </tr>
